...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NEW: PDF Generation Function - using jsPDF direct text rendering with KumarkhaliMJ Bold font
    async function generateInvoicePdf() {
        try {
            const pdf = new window.jspdf.jsPDF('portrait', 'pt', 'a4');

            // --- IMPORTANT: Add the KumarkhaliMJ Bold font ---
            // 'KumarkhaliMJ Bold' is the font name from the converter.
            // 'KumarkhaliMJ Bold-normal.ttf' is the TTF filename given to converter.
            // window.font is the variable name generated by the converter for the base64 font data.
            // The addFileToVFS and addFont functions are crucial for custom font rendering.
            pdf.addFileToVFS('KumarkhaliMJ Bold-normal.ttf', window.font); 
            pdf.addFont('KumarkhaliMJ Bold-normal.ttf', 'KumarkhaliMJ Bold', 'normal');
            pdf.setFont('KumarkhaliMJ Bold', 'normal');
            // --- End Font Setup ---
            ...
            const col1X = 55; // পণ্য
            const col2X = 220; // পরিমাণ (centered)
            const col3X = 380; // একক মূল্য (right aligned)
            const col4X = pdf.internal.pageSize.width - 55; // মোট মূল্য (right aligned)
            const colWidths = [
                160, // পণ্য
                80,  // পরিমাণ
                100, // একক মূল্য
            ...
            cart.forEach(item => {
                ...
                const productNameLines = pdf.splitTextToSize(item.name, colWidths[0] - 5); // product name width adjusted
                ...
                // Allow more space for next content before adding a new page (e.g., 100pt bottom margin)
